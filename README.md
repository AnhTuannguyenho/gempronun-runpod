...
```json
{ "input": {
    "route": "score",            // score | grade | grade_ph | transcribe | pron | tts | health
    "text": "apple",             // văn bản mẫu
    "audio_b64": "<base64>",     // file audio (webm/mp3/wav/m4a...) — KHÔNG cần cho tts
...
### TTS (Kokoro)
```json
{ "input": { "route": "tts", "text": "Hello world", "voice": "af_heart", "lang": "a", "speed": 1 } }
```
Output: `{ok, audio_b64: <wav 24kHz base64>, sr:24000, format:"wav", dur, took}`.
`lang`: `a`=US, `b`=UK English. Giọng: `af_heart, af_bella, am_adam, am_michael, bf_emma, bm_george, ...`
Model Kokoro nạp **lười** lần gọi tts đầu (không làm chậm cold-start chấm điểm); model+vài giọng đã nướng sẵn trong image.
